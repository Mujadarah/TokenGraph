...
var Language = class _Language {
  ...
  */
   ...
   */
  static async load(input2) {
    let binary2;
    if (input2 instanceof Uint8Array) {
      binary2 = input2;
    } else if (globalThis.process?.versions.node) {
      const fs2 = await import("fs/promises");
      binary2 = await fs2.readFile(input2);
    } else {
...
function nodeKind(language, type) {
...
async function run2() {
  await Parser.init({ locateFile: (file) => join(input.assetRoot, file) });
  ...
  let symbolDetails = nodes.map((node) => {
    ...
    return name2 ? {
      ...
      signature: `${kind} ${name2}`
    } : void 0;
  }).filter((value) => Boolean(value)).sort((left, right) => left.name.localeCompare(right.name) || left.startLine - right.startLine);
  const errorNodeCount = tree.rootNode.descendantsOfType("ERROR").length;
  const degradedReason = symbolDetails.length > input.limits.maxSymbols ? "symbol limit exceeded" : void 0;
