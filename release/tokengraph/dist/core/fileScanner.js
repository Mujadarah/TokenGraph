...
const createIgnore = ("default" in ignorePackage ? ignorePackage.default : ignorePackage);
...
function detectFileKind(path, extension, content) {
    ...
    if (extension === ".md" || extension === ".mdx")
        return "doc";
    if (nextRouteForPath(path))
        return "next-route";
    if (extension === ".tsx" || extension === ".jsx")
        return "react-component";
    if ((extension === ".js" || extension === ".mjs") && /\b(?:return|=>)\s*\(?\s*<[A-Z_a-z][A-Za-z0-9.:-]*(\s|>|\/)/.test(content))
        return "react-component";
    return "module";
}
...
function candidatePathsForBase(root, basePath) {
    ...
    const emittedJavaScriptCandidates = extension === ".js"
...
export function resolveProjectImports(root, graph) {
...
export async function scanProjectSignature(root, options) {
    return (await scanProjectFileMetadata(root, options)).scanSignature;
}
export async function scanProjectFileMetadata(root, options) {
    const ignoreMatcher = await loadRootIgnore(root);
    const rows = [];
    const files = [];
    const exclusions = [];
...
export async function scanProjectFile(root, metadata) {
    let content;
    try {
...
export function isSupportedCodeFile(path) {
