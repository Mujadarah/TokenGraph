...
function routeLabel(project, path) {
    ...
    if (appMatch?.[1])
        return `/${appMatch[1].replace(/\/\([^/]+\)/g, "").replace(/\/$/, "")}`;
...
function relatedCode(project, changedFiles) {
    ...
    const routePaths = unique([...changedFiles, ...inbound.map((edge) => edge.filePath)].map((path) => routeLabel(project, path)).filter((route) => Boolean(route)));
    const seen = new Set();
    return {
        affectedFiles: rows
...
function sqlTextForObject(object) {
...
function relatedMemories(memories, text) {
    ...
    return memories
        ...
        const score = (memory.type === "bug" ? 4 : 0) +
...
function reviewWarnings(input) {
    ...
    const warnings = [];
    if (/tenant|tenant_id/.test(text))
        warnings.push("Manual review: tenant isolation changed or appears in affected SQL.");
    if (/\brls\b|policy|using\s*\(/.test(text))
        warnings.push("Manual review: RLS policy behavior is involved.");
    if (/auth|authenticated|auth\.uid/.test(text))
        warnings.push("Manual review: auth-sensitive behavior is involved.");
    if (/audit/.test(text))
        warnings.push("Manual review: audit logging behavior is involved.");
    if (input.changedFiles.some((path) => /migration|supabase|sql/i.test(path)))
        warnings.push("Manual review: database migration changes can affect persisted data.");
    if (input.affectedRoutes.length > 0)
        warnings.push("Manual review: user-facing route exposure is affected.");
    if (input.affectedRules.length > 0)
        warnings.push("Manual review: architecture rule findings overlap changed files.");
    if (input.affectedMemories.some((memory) => memory.type === "bug"))
        warnings.push("Manual review: known bug memories match this change.");
    return unique(warnings);
}
function riskScore(input) {
    const score = input.changedFiles.length * 8 +
