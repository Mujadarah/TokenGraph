...
function classifyTask(task) {
    ...
    if (/\b(fix|bug|error|failing|regression)\b/.test(text))
        return "bug";
    if (/\b(refactor|cleanup|rename|split)\b/.test(text))
        return "refactor";
    if (/\b(sql|database|table|migration|rls|policy|postgres|supabase)\b/.test(text))
        return "database";
    if (/\b(test|spec|coverage)\b/.test(text))
        return "test";
    if (/\b(doc|readme|guide|documentation)\b/.test(text))
        return "docs";
    if (/\b(architecture|design|why|explain)\b/.test(text))
        return "architecture";
    return "feature";
}
...
function rankedFiles(project, terms, includeTests, includeZero = false) {
    return project.files
        ...
        const symbolScore = scoreText(project.symbols
            .filter((symbol) => symbol.filePath === file.path)
            .map((symbol) => `${symbol.name} ${symbol.kind}`)
            .join(" "), terms, 4);
        const importScore = scoreText(project.imports
...
function rankedSql(project, terms) {
    ...
    for (const policy of project.sql.policies) {
        rows.push({
            ...
            score: scoreText(`${policy.name} ${policy.table} ${policy.command ?? ""} ${policy.roles?.join(" ") ?? ""} ${policy.usingExpression ?? ""} ${policy.checkExpression ?? ""}`, terms)
        });
    }
...
export async function buildContextPlan(input) {
    ...
    if (allRelevantFiles.length > relevantFiles.length)
        budgetExclusions.push(`${allRelevantFiles.length - relevantFiles.length} lower-ranked file(s) excluded by profile or explicit file budget.`);
    if (allRelevantTests.length > relevantTests.length)
        budgetExclusions.push(`${allRelevantTests.length - relevantTests.length} lower-ranked test file(s) excluded by profile or explicit file budget.`);
    if (allRelevantSql.length > relevantSql.length)
        budgetExclusions.push(`${allRelevantSql.length - relevantSql.length} lower-ranked SQL object(s) excluded by profile or explicit SQL budget.`);
