...
function shouldPreserveLine(line) {
    ...
    if (!trimmed)
        return false;
    return (ERROR_PATTERN.test(trimmed) ||
...
function confidenceFor(preservedConstraints, recommendedFirstReads, referencedMemoryCount) {
    if (preservedConstraints.length >= 3 && recommendedFirstReads.length > 0)
        return "high";
    if (preservedConstraints.length > 0 || recommendedFirstReads.length > 0 || referencedMemoryCount > 0)
        return "medium";
    return "low";
}
export async function compressContext(input) {
