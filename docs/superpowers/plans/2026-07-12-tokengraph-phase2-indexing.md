...
- [x] **Step 3: Implement the smallest terminal-line fix**

Track curly, parenthesis, and square-bracket depth in `declarationEndLine`, then accept a closing parenthesis or bracket with an optional semicolon once all delimiter depths are balanced:

```ts
const terminalLine = /[};)\]]\s*;?\s*$/;
```

Keep the existing line-by-line balance logic and do not treat delimiters inside quoted strings or comments as declaration terminators.
...
- [x] **Step 1: Write the failing metadata-only freshness test**
...
- [x] **Step 3: Save the refreshed index metadata**
...
- [x] **Step 1: Write the failing concurrent-link test**
...
- [x] **Step 3: Implement the atomic mutation helper**
...
- [x] **Step 4: Add confirm/deprecate concurrency coverage and run tests**
...
- [x] **Step 1: Write the failing overlapping-index test**
...
- [x] **Step 2: Verify the test fails or reproduces a dropped update**
...
- [x] **Step 3: Add a per-root write queue around `ensureProject`**
...
- [x] **Step 4: Run the focused test repeatedly**
...
- [x] **Step 2: Scan changed files for non-ASCII characters**

```powershell
git diff --name-only codex/tokengraph-phase1-security...HEAD | ForEach-Object { if (Select-String -LiteralPath $_ -Pattern '[^\x00-\x7F]' -Quiet) { throw "non-ASCII: $_" } }
```

- [x] **Step 3: Push and open the Phase 2 PR**
