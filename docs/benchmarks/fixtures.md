...
Scoring labels live in the versioned corpus. Independent raw-baseline files, durable memories, and reproducible expected compact references live in `plugins/tokengraph/scripts/benchmark-evidence-v1.json`. The evaluator never builds memories, planner context, compressor text, tracer logs, risk inputs, or wiki inputs from required files, critical constraints, expected tests, or forbidden-file labels.
