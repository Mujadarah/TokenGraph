...
The report publishes an execution-inclusive net as its primary savings metric. A recommended exact slice is not automatically executed: the benchmark charges a real hash-validated slice request/result only when the corpus explicitly declares an unresolved post-lifecycle evidence gap using either an exact indexed symbol or an explicit positive line range. File-only targets, combined locators, unresolved symbols, out-of-scope files, and disallowed targeted reads fail validation; there is no line-1 fallback. This follows the production read-policy state machine and avoids fabricating unnecessary reads. A separate mutation test forces one slice and verifies its bytes against the indexed source. These remain fixture estimates, not provider billing. Live diagnostic estimates label their counterfactual explicitly: `full-index-dump` for context plans and the compatibility savings tool, `task-files-and-memories` for regression risk, `provided-context` for context compression, and `provided-output` for output compression and failure traces. These diagnostics do not replace execution-inclusive benchmark accounting.
...
Task-level failures, bypasses, and the full execution-inclusive distribution remain visible. The current fixture passes with a +172.3-token activated-task median, +38.3-token p25, and 81.5% non-negative activated tasks after charging four hash-validated exact source slices. This deterministic gate does not by itself enable enforced routing; B6 promotion requires schema-v3 host evidence to be reviewed, Stage-0 median latency both below activation median and at most 5 ms, and every other paired-evaluation gate. Schema-v1 and schema-v2 evidence remains historical and non-promoting.
...
This harness proves repeatability against one synthetic local fixture and catches benchmark regressions. It does not prove universal agent output quality, exact billed-token reduction, or equivalent Codex and Claude behavior. Paired host runs on identical tasks and repository states remain separate release evidence. Their verifier runs as the final attested command inside the evaluated Codex turn under a root-deny, network-disabled permission profile with an empty inherited subprocess environment; raw and normalized evidence becomes durable before worktree cleanup.
