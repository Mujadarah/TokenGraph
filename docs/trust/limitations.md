...
Local change capsules do not fetch pull requests or resolve remote-only refs. A pull-request source is a pair of base/head refs that must already exist locally. Capsule construction fails closed beyond 1,024 entries, 4,096 parsed symbols, 256 exact slices, or 512 KiB of aggregate slice text. Individual binary, deleted, or oversized targets remain represented by metadata but do not yield parsed symbols or source slices.
...
Transactional index publication flushes generation and manifest file contents before same-directory atomic rename. POSIX directory synchronization is mandatory. On Windows, Node may reject directory-handle synchronization even on supported local filesystems; TokenGraph accepts only the documented unsupported-operation errors after the file flush and atomic rename. It therefore does not claim independently proven namespace persistence across abrupt power loss on those Windows hosts. It never responds by writing in place or deleting a manifest-selected generation.
...
Plugin Eval scenarios are repository-controlled measurements, not proof of universal quality or provider-token savings. The evaluator complements the deterministic fixture and paired-host tracks; it does not authorize routing promotion. Plugin Eval currently warns when `interface.termsOfServiceURL` is absent. TokenGraph has no canonical terms-of-service URL, so v0.25 documents and accepts that warning instead of inventing a URL or repurposing the privacy policy.
