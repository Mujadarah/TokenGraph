...
function normalizeRule(input: ArchitectureRuleInput): ArchitectureRule {
...
function pushFinding(report: ArchitectureCheckReport, finding: ArchitectureFinding) {
...
function importTarget(edge: ProjectIndex["imports"][number]): string {
...
function isArchitectureRule(value: unknown): value is ArchitectureRule {
  ...
  const candidate = value as Partial<ArchitectureRule>;
  return (
...
function applyRuleChecks(project: ProjectIndex, rules: ArchitectureRule[], checkedFiles: string[], report: ArchitectureCheckReport) {
...
function applySqlWarnings(project: ProjectIndex, report: ArchitectureCheckReport) {
...
async function applyMarketplaceWarnings(root: string, report: ArchitectureCheckReport) {
  try {
    const marketplace = JSON.parse(await readFile(join(root, ".agents", "plugins", "marketplace.json"), "utf8")) as {
      plugins?: Array<{
        ...
        source?: { path?: string };
        policy?: { developmentOnly?: boolean };
      }>;
    };
