...
function buildReleaseReadme(version, benchmarkResults) {
  const executionInclusiveMedian = benchmarkResults.aggregate?.executionInclusiveMedian;
  const executionInclusiveP25 = benchmarkResults.aggregate?.executionInclusiveP25;
  const nonNegativeActivatedRate = benchmarkResults.aggregate?.nonNegativeActivatedRate;
  if (![executionInclusiveMedian, executionInclusiveP25, nonNegativeActivatedRate].every(Number.isFinite)) {
    throw new Error("Published benchmark results are missing current aggregate metrics.");
  }
  const executionInclusiveMedianText = executionInclusiveMedian.toFixed(1);
  const executionInclusiveP25Text = executionInclusiveP25.toFixed(1);
  const nonNegativeActivatedRateText = (nonNegativeActivatedRate * 100).toFixed(1);
...
Claude launches through \`\${CLAUDE_PLUGIN_ROOT}\` and forwards \`\${CLAUDE_PROJECT_DIR}\`. On Codex, each MCP request names its project and thread; the runtime accepts it only when the reviewed lifecycle hook attestation for the matching \`CODEX_THREAD_ID\` names the same project. MCP Roots and the explicit \`TOKENGRAPH_WORKSPACE_ROOT\` compatibility fallback remain supported. Call \`tokengraph_setup\` before project tools; it diagnoses setup without granting filesystem trust.
...
async function writeReleaseMetadata(packageDir, packageJson, version, benchmarkResults) {
  await writeFile(resolve(packageDir, "README.md"), buildReleaseReadme(version, benchmarkResults));
  await writeFile(resolve(packageDir, "package.json"), `${JSON.stringify(buildReleasePackageJson(packageJson), null, 2)}\n`);
}

async function copyInstallablePlugin(packageDir, packageJson, version, benchmarkResults) {
  ...
  await writeReleaseMetadata(packageDir, packageJson, version, benchmarkResults);
...
async function runPackage() {
  ...
  const benchmarkResults = await readJson(resolve(repoRoot, "docs", "benchmarks", "results-current.json"));
  ...
  if (args.release) {
    await copyInstallablePlugin(args.releaseDir, packageJson, version, benchmarkResults);
  ...
  await copyInstallablePlugin(packageDir, packageJson, version, benchmarkResults);
