...
5. Call `tokengraph_task_report({ taskId, root?, disposition: "complete" })` only after the requested outcome and verification are complete. Use `tokengraph_task_report({ taskId, root?, disposition: "pause" })` for approval, missing evidence, blocked setup after task creation, or unfinished work.
