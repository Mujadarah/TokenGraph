...
describe("TokenGraph doctor", () => {
  it("collects a stable read-only report without creating workspace state", async () => {
  ...
  it("reports stale and malformed leases, orphan ledgers, quota pressure, telemetry, missing parsers, and corrupt indexes without repair", async () => {
    ...
    await Promise.all([
      ...
      mkdir(join(root, ".tokengraph", "memory.json.lock"), { recursive: true }),
    ...
    await Promise.all([
      ...
      writeFile(join(root, ".tokengraph", "config.json"), JSON.stringify({ schemaVersion: 4, config: { storage: { writePolicy: "minimal", maxBytes: 1, runsMaxBytes: 1, cacheMaxBytes: 1, vaultMaxBytes: 1, durableMaxBytes: 1 } } })),
      ...
      writeFile(join(root, ".tokengraph", "memory.json.lock", "lease.json"), "not json"),
      ...
      writeFile(join(root, ".tokengraph", "telemetry", "write-aggregates.json"), JSON.stringify({ schemaVersion: 1, days: [{ date: "2026-09-02", sampledPeakRssBytes: 7, classes: { durable: { operationCount: 2, logicalBytes: 12 } } }] })),
    ...
    expect(report.storage).toMatchObject({ quotaPressure: true, writePolicy: "minimal", recentWrites: { operations: 2, logicalBytes: 12, sampledPeakRssBytes: 7 } });
  ...
  it("compares source manifests with the generated release and packaged manifests without inventing a release sibling", async () => {
    ...
    const sourceReport = await collectDoctorReport({ workspace: { status: "blocked", source: "test", blockingReason: "no-workspace" }, pluginRoot: source });
