...
Download `tokengraph-0.22.0.zip` from the [latest GitHub release](https://github.com/Mujadarah/TokenGraph/releases/latest) and extract it. The extracted directory is a standalone marketplace root containing both host catalogs and the installable `tokengraph/` plugin.
...
```bash
claude plugin marketplace add /path/to/tokengraph-0.22.0
claude plugin install tokengraph@tokengraph
```
...
Real-host evidence is reported separately from fixture economics. The first reviewed evaluation completed five counterbalanced ON/OFF pairs for one implementation task with no host or acceptance failures, but promotion is disabled because the frozen coverage and performance gates did not all pass. This one repository does not satisfy multi-repository B6 validation. See the checked [real-host manifest](docs/benchmarks/host-evaluations/2026-07-19-tokengraph-codex-manifest.json) and [real-host report](docs/benchmarks/host-evaluations/2026-07-19-tokengraph-codex-report.md).
